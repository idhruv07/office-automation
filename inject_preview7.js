...
async function run() {
    try {
        ...
        // 2. Meta Row (Letter No & Date)
        // Extract lines that look like: <p>No. IT&SDC/Hardware/2025/AI Server        Dated: 25.08.2025</p>
        const metaRowRegex = /<p>[^<]*No\.\s+(.*?)Dated:\s+(.*?)<\/p>/ig;
        const replacementMetaRow = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 20px; margin-bottom: 20px; font-weight: bold; border-bottom: 2px solid #ccc; padding-bottom: 10px;">
           <div style="text-align: left;">No. $1</div>
           <div style="text-align: right;">Dated: $2</div>
        </div>
        `;
        if (metaRowRegex.test(html)) {
            metaRowRegex.lastIndex = 0;
            html = html.replace(metaRowRegex, replacementMetaRow);
        }

        // 3. Office Note Title (Center it)
        const titleRegex = /<p>[^<]*(<u[^>]*>)?OFFICE NOTE(<\/u>)?[^<]*<\/p>/ig;
        const replacementTitle = `
        <div style="text-align: center; margin-top: 20px; margin-bottom: 30px;">
            <h2 style="text-decoration: underline; font-weight: bold; margin: 0;">OFFICE NOTE</h2>
        </div>
        `;
        ...
        const docRes = await pool.query(
            `INSERT INTO documents (folder_id, title, reference_no, status, owner_type, owner_office_id)
             VALUES (2, 'TEST PREVIEW V7: Perfect Layout & Spacing', 'TEST-007', 'active', 'office', 1)
             RETURNING id`
        );
        ...
        await pool.query(
            `INSERT INTO document_page_versions (page_id, version, html_content, diff_summary, edited_by)
             VALUES ($1, 1, $2, 'Mammoth Extraction Test with Flexbox Meta Row and Centered Title', 103)`,
            [pageId, html]
        );
