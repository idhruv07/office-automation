...
async function run() {
    try {
        ...
        // User requested we use the same FWD note images
        ...
        // Note the 'g' flag in headerRegex to match MULTIPLE times
        ...
        const docRes = await pool.query(
            `INSERT INTO documents (folder_id, title, reference_no, status, owner_type, owner_office_id)
             VALUES (2, 'TEST PREVIEW V6: Split Markers & Images', 'TEST-006', 'active', 'office', 1)
             RETURNING id`
        );
        ...
        await pool.query(
            `INSERT INTO document_page_versions (page_id, version, html_content, diff_summary, edited_by)
             VALUES ($1, 1, $2, 'Mammoth Extraction Test with Real Images and Split Marker', 103)`,
            [pageId, html]
        );
